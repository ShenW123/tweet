...
if len(sys.argv) > 2: #Must have at least 3 arguments 1st: Determines # of tweets, 2nd - n-1th: Determines classes and readfiles, Last: Determine writefile
    if ((sys.argv[0][0] == '-') & (len(sys.argv[0]) > 1)): #if it begins with - and has more than just that
        tweet_number = int(sys.argv[0].split('-')[1])
        if (tweet_number < 0):
            tweet_number = 9999
    else:
    ...
    feature_class = []
    feature_files = []
    for index in range(len(sys.argv) - 2): #So can handle the + 1 to index by reducing length by 1 and skips last argument
        n = sys.argv[index + 1] #So skips the first index 0
        n = n.split(':')
        if len(n) > 1:
